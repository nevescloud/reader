...
import { normCode } from "./util";

const HOST = "live.neves.cloud";
  ...
  async init() {
    this.server.registerTool(
      ...
      async ({ code, content, title }) => {
        ...
        if (c.length < 4) {
          return {
            content: [{ type: "text", text: `"${code}" doesn't look like a reader code. Ask the user for the 5-character code shown on their reader (open ${HOST} on the device).` }],
            isError: true,
          };
        }
        ...
        return {
          content: [{ type: "text", text: `Sent to reader ${c} (update #${v}) — "${t}". It appears on the device within ~3s. Direct link: https://${HOST}/r/${c}` }],
        };
      },
    );
    ...
    this.server.registerTool(
      ...
      async ({ code }) => {
        ...
        const text = s.connected
          ...
          : `No reader is polling code ${c} right now. Ask the user to open ${HOST} on their e-reader and read off the code shown.`;
